...
- `.factory/services.yaml` now uses portable `ROOT="$(git rev-parse --show-toplevel)"` approach for all commands. The hard-coded absolute path has been removed, and `tests/copysnipin/test_factory_portability.py` asserts the old path is absent.
